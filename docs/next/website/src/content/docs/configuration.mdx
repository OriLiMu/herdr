...
On local Unix Ghostty clients, Herdr can send RGB/RGBA pixels and encoded PNG images through temporary files instead of encoding them into terminal output. Applications continue using standard Kitty graphics; Herdr still manages placement, clipping, and visibility. Herdr first checks whether the outer terminal consumes a small test file and keeps inline output if it does not. Other terminals, SSH-launched clients, and nested multiplexers retain inline output. Set `HERDR_KITTY_FILE_TRANSPORT=0` before launching the client to disable this optimization.

For testing only, `HERDR_EXPERIMENTAL_KITTY_PNG_FORWARD=1` on the server preserves eligible quiet PNG uploads as encoded PNG instead of decoding them into pixels. Animation operations decode them when needed; queries and uploads requesting error replies keep full validation. This mode is off by default because corruption inside compressed image data may be detected only downstream, after Herdr has placed the image. It does not forward mutable producer file paths.

For local testing, set `HERDR_EXPERIMENTAL_KITTY_NATIVE_FILES=1` on both the server and client to try file-backed delivery of native RGBA images. Herdr creates its own image snapshot and sends its path instead of pixel bytes to the client; it does not borrow the application's original file. This also enables experimental stable outer image IDs. Unsupported clients or failed transfers fall back to inline delivery. This mode is off by default.

On Linux, `HERDR_EXPERIMENTAL_KITTY_SOURCE_FILES=1` on the server additionally tries to snapshot whole-file, uncompressed native RGBA uploads before reading pixels. This requires a filesystem that supports copy-on-write cloning into Herdr's private `/var/tmp` store. Unsupported filesystems, temporary-file uploads, shared memory, and other formats keep the existing loader. Combine it with native file delivery above to avoid copying pixels between the server and local client. Snapshots are limited to 16 MiB each and 64 MiB total; animation and unsupported clients read their pixels when needed. This experiment is off by default.
